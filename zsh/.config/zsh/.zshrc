...
precmd() { echo }
PROMPT=$'%B%F{red}%(5~|.../%4~|%d)%f%b'
PROMPT+=$'%B%F{blue}%(1j. ↓%j.)%f%b'
...
__conda_setup="$('/home/pi/.local/share/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/pi/.local/share/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/pi/.local/share/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/pi/.local/share/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
